...
public class ReadCSV2 {
	String csvFile = Gui.fileName2.toString();
  ...
  public void run() throws UnsupportedEncodingException {
	...
	try {
		...
		filename = "EnagyOut" + r.nextInt(99) + "_" + (date) + ".txt";
	...
	try {
		...
		while ((line = br.readLine()) != null) {
			...
			GetData2.getdata(oszlop[11].toString());
			...
			for(int i=0; i != GetData2.list.size(); i++)
			{
			if(GetData2.list.get(i).equals(" !!! "))
			{
				GetData2.list.set(i, " !!! " + hibasak.get(i).toString() + " !!! ");
			}
			}
		}
		...
		System.setOut(out);
		System.out.println(other_data_nev.get(i).toString() + ";" + other_data_szamla.get(i).toString() + ";" + other_data_osszeg.get(i).toString() + ";" + GetData2.list.get(i).toString() + ";");
		}
